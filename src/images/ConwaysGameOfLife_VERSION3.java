...
public class ConwaysGameOfLife_VERSION3 extends JFrame implements ActionListener {
    ...
    private int i_movesPerSecond = 20;
    private GameBoard gb_gameBoard;
    private Thread game;

    public static void main(String[] args) {
        // Setup the swing specifics
        JFrame game = new ConwaysGameOfLife_VERSION3();
        game.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        game.setTitle("Conway's Game of Life");
        game.setIconImage(new ImageIcon(ConwaysGameOfLife_VERSION3.class.getResource("/images/logo.png")).getImage());
    ...
    public ConwaysGameOfLife_VERSION3() {
        // Setup menu
    ...
    private class GameBoard extends JPanel implements ComponentListener, MouseListener, MouseMotionListener, Runnable {
        private volatile Dimension d_gameBoardSize = null;
        private volatile CopyOnWriteArrayList<Point> point = new CopyOnWriteArrayList<Point>();
        private volatile boolean[][] gameBoard;

        private int POOL_SIZE = 4; //change this only
        private ExecutorService pool = Executors.newFixedThreadPool(POOL_SIZE);
        private CountDownLatch latch = new CountDownLatch(POOL_SIZE);
        ...
        //removes points that pass the grid border
        ...
        public void run() {
            ...
            while(!game.isInterrupted() && cycles > 0) {
                ...
                resetBoard();
                try {
                    for(int i = 1; i <= POOL_SIZE; i++) {pool.execute(new BladeRunner(i,latch));}
                    latch.await();
                    repaint();
        ...
        private class BladeRunner implements Runnable {
            ...
            public BladeRunner(int i, CountDownLatch c) {
                threadNum = i;
                latch = c;
            }
            ...
            public void run() {
                ...
                int columnRange = threadNum * (gameBoard.length / POOL_SIZE);

                // Iterate through the array, follow game of life rule
                for (int i = 1; i < columnRange - 1; i++) {
